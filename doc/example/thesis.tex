...
\pkuthssinfo{
	...
	ckeywords = {分支过程，Galton-Watson 树，超过程，脊柱分解，双脊柱分解，极限行为},
...
\end{cabstract}
\begin{eabstract}
	This thesis focuses on the limiting theory of Galton-Watson branching processes and a class of superprocesses in the critical regime.
	Properties and relationships between their asymptotic behaviors and the multi-spine theory are considered.
	In particular, we study systematically the spine decompositions and the two-spine decompositions of critical Galton-Watson trees and a class of critical superprocesses, and their Kolmogorov type, Yaglom type and Slack type limit results. 
...
\section{Backgrounds}
    ...
    Typically, there are three different cases to consider: in the supercritical case, the expectation of the population growths exponentially; in the subcritical case, the expectation of the population decreases exponentially; and in the critical case, the expectation of the population stays as a constant. 

    The limiting behavior of Galton-Watson processes are well known, see \cite{AthreyaNey1972Branching} for example. 
    In the critical case, Slacks \cite{Slack1968A-branching} considered Galton-Watson processes with offspring distribution belonging to the domain of attraction of an $\alpha$-stable law with $\alpha \in (1,2]$. 
    He showed that the total population, after an appropriate rescaling and conditioning, convergence weakly to a random variable $\mathbf z^{(\alpha)}$ with Laplace transform $E[e^{-u\mathbf z^{(\alpha)}}] = 1-(1+u^{-\alpha})^{-1/\alpha}$. While $\alpha = 2$, this result is firstly obtained by Yaglom \cite{Yaglom1947Certain}, and therefore, is known as Yaglom's theorem.
    
    It turns out that Slack type result is universal, in the sense that, for almost all the Markovian branching processes mentioned above, similar Slack type weak limit results are true. 
...
\end{table} 
    ...
    For critical superprocesses without second-moment conditions, it is natural to ask whether Slacks type result is valid. Also, for critical superprocesses with second-moment condition, since the methods used by \cite{EvansPerkins1990Measure-valued} and \cite{RenSongZhang2015Limit} are all analytic, it is natural to ask whether an intuitive probabilistic proof exists.
    ...
    Now, in order to be mare precise about all these results and methods, let us first introduce the model considered in this thesis.
...
\end{equation}
	...
	The idea behind the definitions is that $Z_n$ is the number of individuals in the $n$-th generation, and each member of the $n$-th generation gives birth independently to an identically distributed number of children. $\mu(k)=P(\xi_i^n=k)$ is called the offspring distribution. Let $m = E[\xi_i^n]\in (0,\infty)$. It can be verified easily that $M_n:=(Z_n/m^n)_{n\geq 0}$ is a non-negative martingale with respect to the natural filtration of $(Z_n)$.
...
\]
	Therefore, in this case we have as surely that $Z_n = 0$ for $n$ large enough. This also says that $M_\infty = 0$.
...
\]
	...
	We will call $(Z_n)$ a $\mu$-Galton-Watson processes, and say it is subcritical, critical and supercritical with respect to $m < 1$, $m = 1$ and $m >1$. 
	In this thesis, we mainly focus on the asymptotic behavior of the critical branching processes. 
	For the limiting behavior of the subcritical and supercritical cases, we refer our reader to \cite{AthreyaNey1972Branching}.

	For the critical branching processes, the following result is well known:
...
\end{thm}
\end{comment}
	Under a third moment assumption, assertions \eqref{thm:kolmogorov} and \eqref{thm:yaglom} of Theorem \ref{thm: Kolmogrov and Yaglom theorem} are due to Kolmogorov \cite{Kolmogorov1938Zur-losung} and Yaglom \cite{Yaglom1947Certain} respectively.
	Therefore, Theorem \ref{thm: Kolmogrov and Yaglom theorem}(1) is usually called Kolmogorov's result, and Theorem \ref{thm: Kolmogrov and Yaglom theorem}(2) is usually called Yaglom's result.
	...
	Slack \cite{Slack1968A-branching} considered the critical Galton-Watson branching processes without the finite variance condition, and he obtain the following:
\begin{thm}
\label{thm: Slacks result}
...
\]
\end{thm}
	We will call the above Slack's result.
	Note that, while $\alpha = 2$, Slack's result actually degenerate to Kolmogorov's and Yaglom's result. 
	...
	A large part of this thesis is devoted to give a new probabilistic proof of the Kolmogorov type and Yaglom type result for a class of critical superprocesses with finite second moment condition, and to give a proof of the Slack type of result for a class of critical superprocesses without the finite second moment condition. 
...
\begin{exa}
	Suppose that $E=\{x_0\}$ is a space which has only one point. Let $\xi_t \equiv x_0$ be a trivial process. Let the branching mechanism be
\begin{equation}
\label{eq: psi for CSBP}
	\psi(x_0,z):=\psi(z) := -b z + a z^2 + \int_{(0,\infty)}(e^{-zy}-1+zy)\mu(dy) ,\quad z\geq 0,
\end{equation}
	where $b\in \mathbb R, a \geq 0$ and $\mu$ be a measure on $(0,\infty)$ with $\int_{(0,\infty)} y \wedge y^2 \pi(dy) < \infty$. 
...
\]
	...
	It is easy to verify that $(Y_t)$ is also a Markov process, and its corresponding transition probability $(P_y)_{y\geq 0}$ satisfies the following branching property:
\[
	P_y[e^{-\lambda Y_t}] = P_{y_1}[e^{-\lambda Y_t}] P_{y_2}[e^{-\lambda Y_t}],\quad t\geq 0,\lambda \geq 0,
\]  
	where $y = y_1+y_2$ and $y,y_1,y_2\geq 0$. 
	If the branching mechanism takes the form of 
\begin{equation}
\label{eq: branching mechanism for CSBP}
	\psi(z) = z^2, \quad z\geq 0,
\end{equation}
	then $(Y_t)_{t\geq 0}$ is also known as the Feller's diffusion, and it is the solution to the SDE
\[
...
\]
	In this case, the $(\xi,\psi)$-superprocess $\{(X_t);(\mathbb P_\mu)_{\mu \in \mathbb R^d}\}$ is called the super Brownian motion. Let $\mu \in \mathcal M(\mathbb R^d)$ and $f$ be a continuous nonnegative bounded Borel function on $\mathbb R^d$, we have
\[
...
\end{exa}
	...
	Besides its connection with the non-linear PDEs, the other reason that the superprocesses attract probabilistic' attention is that it is the large scale limit of several discrete stochastic particle systems. 
	This includes the branching particle systems \cite{Dawson1993Measure-valued,Li2011Measure-valued,Dynkin1991Branching}, the long-range contact process \cite{MullerTribe1995Stochastic,DurrettPerkins1999Rescaled}, the voter model and Lotka-Volterra model \cite{CoxDurrettPerkins2000Rescaled,CoxPerkins2005Rescaled} and long range percolation \cite{LalleyZheng2010Spatial}. 
	We will not give the full picture in this direction. Instead, we present the following example which says that the large scaling limit of a binary splitting branching Brownian motion is the super Brownian motion with branching mechanism $\psi(z) = z^2$.
	This result will not be directly used in this thesis. 
	We present it here, because it gives an interpretation of the superprocesses, and shows how the superprocesses and the branching processes are connected.

	Here, by the binary splitting branching Brownian motion $(X_t)_{t\geq 0}$, we mean the following model: 
\begin{itemize}
\item
	at the beginning, there are several particles living in $\mathbb R^d$;
\item
	independent of other particles, each particle in the system performs standard Brownian motion and be killed at a constant rate $r>0$;
\item
	independent of other particles, each particle in the system, at the end of its life, dies with no offspring or splits into two new particles, with equal probability $1/2$;
\item
	each particle in the system has a same weight $m>0$; let $t\geq 0$ and $A$ be a measurable subset of $\mathbb R^d$, $X_t(A)$ is the total weight of all the particles positioned in $A$ at time $t$. 
\end{itemize}

	
\begin{thm}[Watanabe, Dawson]
	Fix a $\mu \in \mathcal M(\mathbb R^d)$. For every $n \in \mathbb N$, consider a binary branching Brownian motion $(X_t^n)$ in $\mathbb R^d$, with branching rate $2n$ and particle weights $n^{-1}$, with its initial configuration $X_0^n$ satisfying that $nX^n_0$ is a Poisson random measure on $\mathbb R^d$ with intensity $n\mu$. Then $(X_t^n)_{t\geq 0} \xrightarrow[n\to \infty]{d} (X_t)_{t\geq 0}$ in the Skorokhod space $\mathbb D(\mathbb R_+, \mathcal M(\mathbb R^d))$, based on the weak topology in $\mathcal M(\mathbb R^d)$, where $(X_t)_{t\geq 0}$ is a super Brownian motion with branching mechanism $\psi(z) = z^2$ and initial configuration $\mu$.
\end{thm}

	As mentioned earlier, the analogues result of Theorem \ref{thm: Kolmogrov and Yaglom theorem} and Theorem \ref{thm: Slacks result} were obtained for a lots of Markovian branching processes. And the main interests of this thesis is to prove those results for a large class of general superprocesses. Our approach for the Kolmogorov type and Yaglom type results for the superprocesses are different from the aforementioned works \cite{EvansPerkins1990Measure-valued} and \cite{RenSongZhang2015Limit}, and is more intuitive and probabilistic. 
	The statements of those results for the general superprocesses is quite technical. For the sake of simplicity, in this subsection, we only present our results for the continuous-state branching processes. More precise statements of the theory will be presented in Chapter 3 and Chapter 5 later. We will also give some intuitions of our methods in the next section.
...
\]
	Taking derivative for $\lambda$ on the both side, and let $\lambda = 0$, we get
\[
...
\]
	...
	So we say the CSBP $(Y_t)$ is subcritical, critical, supercritical with respect to $b>0$, $b=0$ and $b <0$.  
...
\]
\end{thm}
	The Slack type result for CSBP are due to \cite{KyprianouPardo2008Continuous} and \cite{RenYangZhao2014Conditional}:
\begin{thm}
	...
	Suppose that $\psi(\lambda)= \lambda^{1+\alpha}L(1/\lambda)$ where $\alpha \in (0,1]$ and $L$ is slowly varying at infinity. Then $F(t):= P_1(Y_t >0)$ convergences to $0$ as $t\to \infty$, and is regularly varying with index $-1/\alpha$. Furthermore, for each $x>0$ and $y\geq 0$, it holds that
\[
...
In my work with collaboration with Ren and Song \cite{RenSongSun2018A-2-spine}, we propose a $k(k-1)$-type size-biased $\mu$-Galton-Watson tree equipped with a two-spine skeleton, which serves as a change-of-measure of the original $\mu$-Galton-Watson tree;
and with the help of this two-spine technique, in the next chapter, we give a new probabilistic proof of Theorem \ref{thm: Kolmogrov and Yaglom theorem}(2), i.e. Yaglom's theorem.
...
\end{itemize}
	We refer to $\mathbb N$ the Kuznetsov measure for the CSBP. And with some abuse of the notations, we will always assume that our CSBP $\{(Y_t); P_x\}$ is given by $Y_t = \mathcal N[w_t], t\geq 0$ for some Poisson random measure $\{\mathcal N;P_x\}$ with intensity $x\mathbb N$.
...
\]
	...
	Here the set $\mathcal D$ is the set of the time of all the immigration events. 
	...
	In Chapter 3, we develop this theory further to include other type of size-biased transformation which may not be Doob's martingale transformation: Let $F$ is a functional for path $(w_s)_{0\leq s\leq t}$ where $w \in \mathbb D$. Suppose that this functional satisfies that $\mathbb N[F(w)] \in (0,\infty)$. Then from the mean formula for the Poisson random measure, we have
\[
...
\end{itemize}
	If we take $F(w)= w_t$, then it will be proved in Chapter 3 that the process $\{(Z_s)_{0\leq s\leq t}; Q_x^{(t)}\}$ given in \eqref{eq: immigrates for classical spine decomposition for CSBP} has the exactly the same law as $\{(w_s)_{0\leq s\leq t}; \mathbb N^{w_t}\}$. In another word, if we know $\{Y, Z, n; Q_x^{(t)}\}$ is a spine representation of $P_x^{Y_t}$, then $\{Y, Z; Q_x^{(t)}\}$ is a size-biased representation of $P_x^{Y_t}$. The following theorem explained the naming: 
...
\end{thm}
	...
	The reason that those decompositions for size-biased CSBP is useful for proving Yaglom type results are similar to that for the Galton-Watson branching processes. 
...
\]
	where on the the right side process $Z'$ is a copy of $Z$, $U$ is an uniform distributed random variable in $[0,1]$, and $Z$, $Z'$ and $U$ are independent.
  	Note that $Z_t$ has the law same as $\{w_t;\mathbb N^{w_t}\}$, and $\tilde Z_t$ has the law same as $\{w_t;\mathbb N^{w^2_t}\}$. This actually implies that $\tilde Z_t$ is the size-biased transform of $Z_t$. 
...
\section{Organization of the thesis}
    ...
    In Chapter 2, we will translated this intuition into mathematics. This is useful both for a new point of view on Yaglom type theorem and as a new application to multi-spine theory. our method is generic in the sense it can apply to much more complicated branching systems such that superprocesses. 
    ...
    In that chapter 4, we consider the characteristic function of Superprocesses. We proof that the characteristic exponent of $\langle X_t,f\rangle$ satisfies a non-linear complex-valued mild PDE where $(X_t)_{t\geq 0}$ is a general non-persistent superprocess and $f$ is a testing function. 
    This is more general than the classical theory about the Laplace exponents of a superprocess satisfying a non-linear real-valued mild PDE, because we allow our testing function $f$ taking both non-negative and negative values. The general spine decomposition theorem in Chapter 4 is used to proof this result. In a follow-up work \cite{RenSongSunZhao2019Stable} in collaboration with Yan-Xia Ren, Renming Song and Jianjie Zhao, we used this result to proof several stable central limit theorems for supercritical super Ornstein-Uhlenbeck processes.
    
    Chapter 5 is based on the work \cite{RenSongSun2018Limit} in collaboration with Yan-Xia Ren and Renming Song. In that chapter, we established Slack type results for a class of critical superprocesses with spatially dependent stable branching.
    Using the general spine theory for the superprocess developed in Chapter 3, we could establish the vanishing speed of survival probability. 
